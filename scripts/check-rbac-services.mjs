...
const guard=app.slice(app.indexOf('  const ModuleProtectedRoute = useCallback'),app.indexOf('  // Public Route wrapper'));
...
function GuardHarness({codes,moduleCode,staff=false,superuser=false}){const isAuthenticated=true;const modulesLoaded=true;const user={is_staff:staff,is_superuser:superuser};const userModules=codes;${guard}return <ModuleProtectedRoute moduleCode={moduleCode}><p>Protected content</p></ModuleProtectedRoute>}
window.renderGuard=props=>root.render(<MemoryRouter key={++key} initialEntries={[props.path]}><GuardHarness {...props}/></MemoryRouter>);
