...
try {
 ...
 assert.equal(await page.getByRole('heading',{name:'Database cleaning',exact:true,includeHidden:true}).count(),0);
 assert.equal(await page.getByRole('button',{name:'Refresh tables',exact:true,includeHidden:true}).count(),0);
 assert.equal(await page.getByRole('button',{name:/^Delete (data|table)/,includeHidden:true}).count(),0);
 ...
 assert.deepEqual(errors,[]);console.log('Passed: typography, 65/35 layout, incident priority, service selection, S3 data, activity details, tabs, database cleaning disabled, task draft, focus ring, mobile overflow, time range and failed-data states.');
