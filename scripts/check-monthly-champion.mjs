...
const candidate = {user_id:'1',user:{id:'1',name:'Avery Engineer',email:'avery@example.test'},rank:1,score:100,requests:12,successful_requests:12,success_rate:100,active_days:3,modules:2,breakdown:{successful_requests:50,request_success_rate:30,active_days:20}};
const method={version:'ai-request-engagement-v2',weights:{successful_requests:50,request_success_rate:30,active_days:20},eligibility:'Active account with at least one successful AI request.',description:'AI request engagement only.',limitations:'Request success does not prove verified business outcomes.'};
const mock=`const candidate=${JSON.stringify(candidate)},method=${JSON.stringify(method)};export default {
getMonthlyChampion:async(year,month)=>{if(window.fail)throw new Error('unavailable');const now=new Date();const closed=year<now.getUTCFullYear()||month<now.getUTCMonth()+1;return {year,month,fingerprint:'preview',scope:'All organizations',can_publish:!window.reader,period:{closed},candidates:window.empty?[]:[candidate],methodology:method,publication:window.award||null,period_published:!!window.award,history:window.award?[window.award]:[]}},
...
try{
 ...
 await page.evaluate(()=>{window.reader=false;window.empty=true});await page.getByRole('button',{name:'Refresh monthly champion'}).click();await page.getByRole('heading',{name:'No eligible candidate',exact:true}).waitFor();
