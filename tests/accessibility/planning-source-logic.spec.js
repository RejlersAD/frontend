...
test('source logic previews an explicit calendar and opens a separate calculated draft with stage colors and source comparison', async ({ page }) => {
  ...
  await scheduleWorkspace(page).locator('.psl-summary summary').click()
  await expect(scheduleWorkspace(page).locator('.psl-summary')).toContainText('22 Dec 2026')
  await expect(scheduleWorkspace(page).locator('.psl-summary')).toContainText('65 days as printed')
  await expect(scheduleWorkspace(page).locator('.psl-summary')).toContainText('67 working days')
  await expect(scheduleWorkspace(page).locator('.psl-summary')).toContainText('partial network')
  ...
  await scheduleWorkspace(page).getByRole('button', { name: 'View original source version', exact: true }).click()
