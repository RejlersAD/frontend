...
test('new request validates its identifier and preview warnings return to the relevant step', async ({ page }) => {
  ...
  await gotoStep(page, 'Approval & submission')
...
test('new request adds supplier shortlist, updates pricing and keeps draft saves on one record', async ({ page }) => {
  ...
  await expect(page.getByLabel('Line item 1 VAT', { exact: true })).toHaveCount(0)
  ...
  await expect(page.locator('.prf-sp-grand-total')).toContainText('400,000.00')
  await expect(page.locator('.prf-sp-budget')).toContainText('600,000.00')
  ...
  await workspace(page).getByRole('button', { name: 'Save', exact: true }).first().click()
  ...
  expect(state.record).toMatchObject({ pr_number: 'RAD-PRJ-PR-9001_2026', supplier_name: 'Alfanar Engineering LLC', total_price: 400000, net_total_excl_vat: 400000, currency: 'AED' })
  ...
  expect(state.record.price_remarks_data.line_details[0]).toMatchObject({ budget: '1000000' })
  ...
  await expect(page.locator('.prf-sp-grand-total')).toContainText('2,500.00')
  await workspace(page).getByRole('button', { name: 'Save', exact: true }).first().click()
  ...
  expect(state.submissions).toEqual([])
  // The API normalizes line items. Vendor and budget must survive through
...
test('edit keeps saved request fields, supplier changes update the document and quantity updates persist', async ({ page }) => {
  ...
  await expect(page.locator('.prf-sp-grand-total')).toContainText('3,000.00')
  await workspace(page).getByRole('button', { name: 'Save', exact: true }).first().click()
...
test('a new PR registers with only its identifier while business details remain warnings', async ({ page }) => {
  ...
  await preview(page).getByRole('tab', { name: /^Validation/ }).click()
  await expect(preview(page)).toContainText('No vendors are shortlisted.')
  await expect(preview(page)).toContainText('Purchase recommendation is missing.')
  await page.getByRole('button', { name: 'Send for Approval', exact: true }).first().click()
  await page.getByRole('dialog', { name: 'Confirm action', exact: true }).getByRole('button', { name: 'Yes', exact: true }).click()
  await expect.poll(() => state.submissions.length).toBe(1)
...
test('all five form steps expose accessible names and readable contrast', async ({ page }) => {
  const state = await open(page, { edit: true })
  const audits = []
  for (const name of ['Request', 'Supplier & pricing', 'Business justification', 'Documents', 'Approval & submission']) {
