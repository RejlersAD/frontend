...
test('cleared introduction stays blank in current exports and after saving until explicitly reset', async ({ page }) => {
  ...
  await expect(introduction(page)).toHaveValue(standard(state.vendors[0].name))
  await expect.poll(() => lastPdf(state)?.contact_persons).toEqual(contacts)
  await page.getByRole('button', { name: 'Save changes', exact: true }).first().click()
  await expect(page.getByRole('heading', { name: 'Purchase Orders', exact: true })).toBeVisible()
  expect(state.acceptedWrites).toHaveLength(2)
  expect(state.acceptedWrites[1].body).toEqual({ contact_persons: contacts })
  await editOrder(page)
  await expect(introduction(page)).toHaveValue(standard(state.vendors[0].name))
  expect(lastPdf(state).contact_persons).toEqual(contacts)
  clean(state)
...
test('approved commercial locks also protect custom introduction and reset', async ({ page }) => {
...
for (const [name, custom] of [['empty', ''], ['whitespace', ' \n\t '], ['unset', undefined]]) {
  test(`legacy document preview ${name === 'unset' ? 'retains the default for unset text' : `omits the entire introduction paragraph for ${name} text`}`, async ({ page }) => {
    ...
    await page.addInitScript(value => { window.approvalFixture = value }, {
      ...
      po: { id: orderFormId, po_number: orderFormNumber, title: 'Company scope introduction case', description: '<p>Engineering scope remains visible</p>', vendor_name: 'Synthetic Seller', items: [], contact_persons: custom === undefined ? {} : { order_introduction: custom } },
    })
