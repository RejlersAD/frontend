...
const currentViewer = page => preview(page).getByRole('region', { name: 'Current purchase order PDF preview', exact: true })
...
test('selecting a recommendation prefills the order and spreadsheet edits update the live document', async ({ page }) => {
  ...
  await expect.poll(() => Number(lastSnapshot(state)?.tax_amount)).toBe(20000)
  ...
  await expect(workspace(page).locator('[name="tax_amount"]')).toHaveValue('20000')
  await expect.poll(() => Number(lastSnapshot(state)?.vendor)).toBe(21)
  ...
  await expect.poll(() => Number(lastSnapshot(state)?.total_amount)).toBe(2625)
...
test('preview zoom, fit width and PDF export use the current unsaved document', async ({ page }) => {
  const state = await open(page)
  await selectPR(page)
  const viewer = currentViewer(page)
  await expect(viewer.getByRole('img')).toBeVisible({ timeout: 30000 })
  const before = await viewer.getByRole('img').boundingBox()
  await viewer.getByRole('button', { name: 'Zoom in', exact: true }).click()
  await expect.poll(async () => (await viewer.getByRole('img').boundingBox()).width).toBeGreaterThan(before.width)
  await viewer.getByRole('button', { name: 'Fit width', exact: true }).click()
  await expect.poll(async () => Math.abs((await viewer.getByRole('img').boundingBox()).width - before.width)).toBeLessThan(2)
  const requestCount = state.requests.filter(request => request.path.endsWith('/preview-document/')).length
  ...
  expect(state.requests.filter(request => request.path.endsWith('/preview-document/'))).toHaveLength(requestCount)
  expect(saves(state)).toEqual([])
  verifyIsolation(state)
})
...
test('desktop and mobile retain the sidebar and provide a usable editor with a full height preview', async ({ page }) => {
  ...
  await expect.poll(() => Number(lastSnapshot(state)?.total_amount)).toBe(1260000)
...
test('editing an order stays inside the application content when the sidebar collapses or the viewport changes', async ({ page }) => {
  ...
  await expect(currentViewer(page).getByRole('img')).toBeVisible({ timeout: 30000 })
...
test('VAT treatment needs confirmation and distinguishes inclusive, exclusive and no-VAT prices without automatic saves', async ({ page }) => {
  ...
  await expect.poll(() => Number(lastSnapshot(state)?.net_amount)).toBe(95.24)
