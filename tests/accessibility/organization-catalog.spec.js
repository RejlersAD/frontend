...
test('profile keeps stored department and title when the chart loads and unrelated fields are saved', async ({ page }) => {
  ...
  await expect(page.getByLabel('Organizational role / Job title', { exact: true })).toHaveAttribute('readonly', '')
  await expect(page.getByRole('button', { name: 'Reporting Manager', exact: true })).toBeDisabled()
  ...
  expect(state.writes[0].body).not.toContain('name="department"')
  expect(state.writes[0].body).not.toContain('name="job_title"')
  expect(state.writes[0].body).not.toContain('name="manager_id"')
