...
test('saved schedule exposes activity-specific duration citation separately from deliverable references', async ({ page }) => {
  ...
  const taskTitle = state.records[17].simplePlan.tasks[1].title
  const evidence = workspace(page).getByRole('table', { name: 'Activity source evidence', exact: true }).getByRole('row').filter({ hasText: taskTitle })
  await expect(evidence).toContainText('Source duration')
  await expect(evidence.locator('.pse-reference')).toHaveAttribute('title', /Approved MDR\.xlsx · Schedule · Row 12/)
  await evidence.getByRole('button', { name: `Show evidence for ${taskTitle}`, exact: true }).click()
  const detail = workspace(page).getByRole('region', { name: `Evidence for ${taskTitle}`, exact: true })
  await expect(detail).toContainText('Approved MDR.xlsx · Schedule · Row 12')
  await expect(detail).toContainText('Original Duration: 8 working days')
