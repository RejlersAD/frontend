...
import { AlertTriangle, CheckCircle2, FileText, GitBranch, Pencil, RefreshCw } from 'lucide-react'
...
const labels = {
  ...
  parallel_workflow_review_required: 'Parallel deliverables need a reviewed sequence or capacity basis',
  unconnected_workflow_start: 'Review open workflow starts',
  terminal_branch: 'Review terminal workflow branches',
}
const resolutions = {
  ...
  parallel_workflow_review_required: 'Review the common predecessors and first-stage durations. Apply justified sequence changes or record the resource and duration basis for keeping these deliverables in parallel, then calculate and review the schedule.',
  unconnected_workflow_start: 'Review whether the workflow needs an external predecessor. Independent starts may be intentional.',
  terminal_branch: 'Review whether the workflow must feed another deliverable or milestone. Final deliverables may intentionally have no successors.',
}
const logicReviewIssues = new Set(['parallel_workflow_review_required', 'unconnected_workflow_start', 'terminal_branch'])
...
function IssueCard({ issue, tasks, locked, busy, onEdit, onInputs, onVerifySources, onEvidence, onSources, onReviewLogic, hideActivities }) {
  ...
  const logicIssue = logicReviewIssues.has(issue.code)
  ...
  return <article className="psc-issue">
    ...
    {logicIssue && onReviewLogic && <div className="psc-actions"><button type="button" disabled={busy} onClick={onReviewLogic}><GitBranch size={15} />Review schedule logic</button></div>}
    ...
    {!rows.length && !needsInputs && !logicIssue && <p className="psc-resolution">Check again to load the latest activity details.{!isTimingWarning(issue) && ' Review the named issue before submitting.'}</p>}
  </article>
}
IssueCard.propTypes = { issue: PropTypes.object.isRequired, tasks: PropTypes.array.isRequired, locked: PropTypes.bool, busy: PropTypes.bool, onEdit: PropTypes.func.isRequired, onInputs: PropTypes.func.isRequired, onVerifySources: PropTypes.func, onEvidence: PropTypes.func, onSources: PropTypes.func, onReviewLogic: PropTypes.func, hideActivities: PropTypes.bool }

export default function PlanningScheduleChecks({ plan, tasks, locked, busy, onEdit, onInputs, onRefresh, onVerifySources, onEvidence, onSources, onReviewLogic }) {
  ...
  return <section className="planning-schedule-checks" aria-label="Schedule issues">
    ...
    {issues.map((issue, index) => <IssueCard key={`${issue.code}-${index}`} issue={issue} tasks={tasks} locked={locked} busy={busy} onEdit={onEdit} onInputs={onInputs} onVerifySources={onVerifySources} onEvidence={onEvidence} onSources={onSources} onReviewLogic={onReviewLogic} hideActivities={Boolean(negative && ['contract_finish_overrun', 'contractual_finish_overrun'].includes(issue.code)) || logicReviewIssues.has(issue.code)} />)}
  </section>
}
