...
const IntegrationsExportsPanel = ({ projectId, versionId, canManage, onNotice }) => {
  ...
  const download = async format => {
    ...
    try {
      ...
      const filename = disposition.match(/filename="?([^";]+)"?/i)?.[1] || `schedule.${format}`
      ...
      await load(true)
      onNotice('success', `${format.toUpperCase()} export generated and checksum-audited.`)
    } catch (error) {
  ...
  return (
    <div className="p-4 space-y-5 bg-slate-50/60">
      ...
      <section><h3 className="font-semibold text-slate-700 mb-2">Version-native exports</h3><div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3">{EXPORTS.map(item => { const Icon = item.icon; return <button key={item.format} onClick={() => download(item.format)} disabled={Boolean(busy)} className="text-left bg-white border rounded-xl p-4 hover:border-violet-300 hover:shadow-sm disabled:opacity-50"><div className={`w-9 h-9 ${item.tone} text-white rounded-lg flex items-center justify-center`}><Icon className="w-5 h-5" /></div><div className="font-semibold text-slate-800 mt-3">{item.title}</div><p className="text-xs text-slate-500 mt-1">{item.description}</p></button> })}</div></section>
