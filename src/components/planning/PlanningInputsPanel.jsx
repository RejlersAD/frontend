...
const sourceText = fact => {
  ...
  return [fact.source_filename || (locator.source === 'project_record' ? 'Project input' : fact.extraction_method === 'manual' ? 'Planner input' : 'Source document'), location].filter(Boolean).join(' · ')
}
...
export default function PlanningInputsPanel({ project, enterpriseProject, contract, loadingContract, files, uploading, analyzing, creatingProgrammatic = false, busy = false, analysisRevision, analysisRunId = null, analysisResult, programmaticSummary, documentDeliverables = [], uploadCategory, onUploadCategory, onUpload, onDeleteFile, onAnalyze, onCreateProgrammatic, onSaved, onOpenWorkBreakdown, onOpenIntelligencePreview, onPreviewStaleChanged, onReviewStateChanged, reviewRequest = 0, onReadinessChanged, onBack, onAiSettings, onRevealInputs, onIntelligenceLoaded, hidden = false, simple = false, generateSchedule = false, idPrefix = 'planning' }) {
  const inputIds = {
    form: `project-${idPrefix}-inputs-form`, methodHeading: `${idPrefix}-method-heading`,
    documentsHeading: `${idPrefix}-documents-heading`, start: `${idPrefix}-project-start`,
    end: `${idPrefix}-project-end`, duration: `${idPrefix}-calculated-duration`,
    reviewHeading: `${idPrefix}-ai-heading`, reviewTitle: `${idPrefix}-review-title`,
  }
  ...
  const editingDisabled = saving || analyzing || creatingProgrammatic || preparingUpload || busy
  ...
  const loadReview = useCallback(async () => {
    ...
    try {
      const run = analysisRunId
        ...
        : await latestCompletedRun(project.id, controller.signal)
  ...
  const save = async event => {
    ...
    try {
      ...
      const response = project?.id ? !hasChanges ? { data: project } : await apiClient.patch(PLANNING_ENDPOINTS.project(project.id), payload) : await apiClient.post(PLANNING_ENDPOINTS.projects, {
  ...
  useEffect(() => { onReviewStateChanged?.({ conflicts: review.conflicts.length, unavailable: unknown, runId: review.run?.id }) }, [review.conflicts.length, review.run?.id, unknown, onReviewStateChanged])
  const openReview = () => { setReviewNotice(''); setReviewOpen(true) }
  return <div className={`planning-inputs${simple ? ' planning-inputs-simple' : ''}`} hidden={hidden}>
    ...
    <div className="pln-columns"><div className="pln-main-column">
      ...
      <section className="pln-card pln-registration" aria-label="Project scope">
        <form id={inputIds.form} onSubmit={save} onInvalid={onRevealInputs} aria-busy={saving}>
          <fieldset disabled={editingDisabled}>
            ...
            <div className="pln-scope-dates"><label htmlFor={inputIds.start}><span>Project start date <span className="pln-required" aria-hidden="true">*</span></span><input id={inputIds.start} aria-label="Project start date" required type="date" value={draft.effective_date} max={draft.planned_end_date || undefined} disabled={baselineLocked || Boolean(project && (loadingContract || !contract))} onChange={change('effective_date')} /></label><label htmlFor={inputIds.end}><span>Project end date <span className="pln-required" aria-hidden="true">*</span></span><input ref={endDateRef} id={inputIds.end} aria-label="Project end date" required type="date" value={draft.planned_end_date} min={draft.effective_date || undefined} disabled={baselineLocked || Boolean(project && (loadingContract || !contract))} onChange={change('planned_end_date')} /></label><div className="pln-duration-field"><label htmlFor={inputIds.duration}>Calculated project duration</label><output id={inputIds.duration} htmlFor={`${inputIds.start} ${inputIds.end}`} className="pln-duration" aria-live="polite"><Calculator size={17} aria-hidden="true" /><span><strong>{duration ? `${duration.days.toLocaleString('en-GB')} calendar ${duration.days === 1 ? 'day' : 'days'}` : 'Select project dates'}</strong><small>{duration ? `${duration.months.toLocaleString('en-GB', { maximumFractionDigits: 2 })} calendar months` : 'Calculated automatically'}</small></span></output></div></div>
