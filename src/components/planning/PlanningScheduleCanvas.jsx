  ...
  return <section ref={canvasRef} className="schedule-canvas" aria-label="Master schedule workspace">
    <header className="sc-commandbar" aria-label="Schedule toolbar">
      ...
      <div className="sc-command-actions">
        {blockers.length > 0 && tab !== 'assurance' && <button type="button" className="sc-issues-button" onClick={() => setTab('assurance')}><AlertTriangle size={15} />Resolve {blockers.length} {blockers.length === 1 ? 'issue' : 'issues'}</button>}
...
PlanningScheduleCanvas.propTypes = {
  ...
  checksOpenRequest: PropTypes.number, checking: PropTypes.bool,
}
