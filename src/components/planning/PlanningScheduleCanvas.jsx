...
import PlanningIntelligentSequence from './PlanningIntelligentSequence'
...
import { dateDisplayTask, dateEvidenceLabel, missingDateLabel } from '../../utils/planningDateEvidence'
...
export default function PlanningScheduleCanvas({ plan, tasks, disciplines, saving, locked, onEdit, onEmployee, onAdd, onInputs, onAnalyze, onRebuild, onRefresh, onSave, onNewVersion, onCompare, onApproval, approvalDisabled, approvalLabel, selectedVersionId = 'current', onVersionChange, onAddWorkstream, onOpenAdvanced, onOpenCreatedSchedule, onBuildSchedule, onCalculate, onValidate, onActivateVersion, buildingSchedule = false, onVerifySources, checksOpenRequest = 0, checking = false }) {
  ...
  const [intelligentSequenceOpen, setIntelligentSequenceOpen] = useState(false)
  ...
  const canProposeSequence = !contextReadOnly && tasks.length > 0 && plan.permissions?.can_propose_sequence === true
  ...
  return <section ref={canvasRef} className="schedule-canvas" aria-label="Master schedule workspace">
    <header className="sc-commandbar" aria-label="Schedule toolbar">
      ...
      {isGrid && <>
        <select className="sc-level-select" aria-label="Schedule detail level" value={viewLevel} onChange={event => { setViewLevel(event.target.value); setSelectedId(null) }}><option value="project">L1 Project</option><option value="wbs">L2 WBS</option><option value="activities">L3 Activities</option></select>
      ...
      <div className="sc-command-actions">
        {isWorkingPlanArea && canProposeSequence && <button type="button" className="sc-intelligent-sequence" disabled={saving || checking || buildingSchedule} onClick={() => setIntelligentSequenceOpen(true)}><Sparkles size={15} /><span>Generate logic & sequence</span></button>}
        {isWorkingPlanArea && hasApprovedProfile && !contextReadOnly && !canProposeSequence && <button type="button" className="sc-build" aria-label="Generate plan" disabled={saving || checking || buildingSchedule} onClick={() => setBuildOpen(true)}><Sparkles size={15} /><span>Generate plan</span></button>}
    ...
    {isGrid && <>
      <div className={`sc-body ${selected ? 'has-selection' : ''}`}>
        <PrimaveraActivitiesGantt plan={plan} tasks={tasks} disciplines={disciplines} search={search} discipline={discipline}
          criticalOnly={criticalOnly} display={display} level={viewLevel} zoom={zoom} fitTimeline={fitTimeline} selectedId={selectedId} locked={locked}
    ...
    {intelligentSequenceOpen && <PlanningIntelligentSequence key={`${plan.project_id || plan.project?.id}:${plan.revision}`} projectId={plan.project_id || plan.project?.id} revision={plan.revision} onClose={() => setIntelligentSequenceOpen(false)} onApplied={result => { setIntelligentSequenceOpen(false); setSelectedId(null); setTab('activities'); onOpenCreatedSchedule?.(result) }} />}
