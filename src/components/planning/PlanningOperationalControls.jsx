...
function ReviewReport({ data, busy, readOnly, dirty, onCommand, onView }) {
  ...
  const fields = [['planned_progress_pct', 'Planned progress', '%'], ['progress_pct', 'Actual progress', '%'], ['remaining_progress_pct', 'Remaining progress', '%'], ...(data.permissions?.can_view_costs === true ? [['bac', 'Budget at completion (BAC)', ''], ['planned_value', 'Planned value', ''], ['earned_value', 'Earned value', ''], ['actual_cost', 'Actual cost', ''], ['spi', 'SPI', ''], ['cpi', 'CPI', ''], ['schedule_variance', 'Schedule variance', ''], ['cost_variance', 'Cost variance', ''], ['eac', 'Estimate at completion (EAC)', ''], ['etc', 'Estimate to complete (ETC)', ''], ['vac', 'Variance at completion', '']] : [])]
  ...
  return <section className="poc-section" aria-label="Review operational report"><header><div><h3>Review & publish</h3><p>Revision {report.revision} · {label(report.status)} · Data date {label(report.data_date)}</p></div></header>
    ...
    <section aria-label="Activity progress tracking"><h4>Activity progress</h4><p>Actual progress follows the approved earning method. Remaining progress is the unearned percentage; it is separate from the remaining working days used to forecast dates.</p>
      <div className="poc-table" role="region" aria-label="Planned actual and remaining progress" tabIndex={0}><table><thead><tr><th>Activity</th><th>Planned %</th><th>Actual %</th><th>Remaining %</th></tr></thead><tbody>{list(report.preview?.activity_comparisons).map(row => <tr key={row.activity_id}><th scope="row">{row.external_id || row.activity_id}<small>{row.name || list(data.activities).find(activity => String(activity.id) === String(row.activity_id))?.name}</small></th><td>{value(row.planned_progress_pct, '%')}</td><td>{value(row.physical_progress_pct, '%')}</td><td>{value(row.remaining_progress_pct, '%')}</td></tr>)}{!list(report.preview?.activity_comparisons).length && <tr><td colSpan={4}>No activity progress is available for this report.</td></tr>}</tbody></table></div>
    </section>
    {data.permissions?.can_view_costs === true && <details><summary>Project control calculations</summary><p>BAC is the approved activity budget total. SPI = earned value / planned value. CPI = earned value / actual cost. EAC projects the final cost at the current cost efficiency; ETC = EAC − actual cost. These statistical estimates use unrounded inputs and require complete approved budgets, progress and cost coverage.</p></details>}
