...
import { receivableCustomer, receivableInvoiceRoute, receivableNumber, receivableRawValue, receivableReadable } from './financeReceivablesPresentation';
...
function RegisterTotal({ metric, descriptionId }) {
  ...
  return <td className="ar-customer-register-number" title={metric?.reason || (value === null ? 'Amount not recorded' : `${currencyLabel(metric?.currency)} ${formatAmount(value)}`)}>
    {formatAmount(value)}{metric?.partial && <abbr title={metric.reason || `${missing === null ? 'Some' : missing} invoice amounts are missing; this is a recorded subtotal.`} aria-describedby={descriptionId}>*</abbr>}
  </td>;
}
RegisterTotal.propTypes = { metric: PropTypes.object, descriptionId: PropTypes.string.isRequired };

export default function CustomerInvoicesSection({ currency = '', company = '', asOf = '', paymentStatus = '', onPaymentStatusChange, sectionRef, refreshKey = 0, enabled = true, dataScope = 'finance', onSnapshotChange }) {
  const headingId = useId();
  const noteId = useId();
  const scope = JSON.stringify([currency, company, refreshKey, dataScope, asOf, paymentStatus]);
  ...
  useEffect(() => {
    ...
    fetchRegister({ currency, company, as_of: asOf, payment_status: paymentStatus, page, page_size: PAGE_SIZE, ordering }).then(data => {
    ...
    return () => { sequence.current += 1; };
  }, [enabled, currency, company, asOf, paymentStatus, page, ordering, requestKey, dataScope]);
  ...
  const workbookMode = data?.source?.mode === 'workbook' || rows.some(row => row.source_snapshot);
  ...
  return <section ref={sectionRef} tabIndex={-1} className="ar-customer-register" data-testid="customer-invoices-section" aria-labelledby={headingId} aria-busy={loading}>
    <header className="ar-customer-register-heading"><h2 id={headingId}>Customer invoices</h2><span title={workbookMode ? 'Original Invoice Amount and Actual Payment Received retain each source row currency. Inv Amt. (AED) contains recorded AED values.' : 'Invoice amounts and payments use the selected original currency. The AED column shows recorded AED invoice values.'}>{workbookMode && selectedCurrency === 'AED' ? 'All original currencies · Recorded AED amounts' : `Invoice currency: ${currencyLabel(selectedCurrency)} · Home currency: ${homeCurrency}`}<InformationCircleIcon aria-hidden="true" /></span>{onPaymentStatusChange && <label className="ar-register-status-filter">Payment status<select aria-label="Customer invoice payment status" value={paymentStatus} onChange={event => onPaymentStatusChange(event.target.value)}><option value="">All statuses</option>{[['new', 'New'], ['overdue', 'Overdue'], ['pending', 'Pending'], ['partial', 'Partial'], ['paid', 'Paid'], ['cancelled', 'Cancelled']].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>}</header>
    <div className="ar-customer-register-panel">
      <div className="ar-customer-register-scroll" tabIndex={0} role="region" aria-label="Customer invoice register table">
        <table className="ar-customer-register-table" data-table-typography="preserve" aria-label="Customer invoices" aria-describedby={partial ? noteId : undefined}>
          <caption className="sr-only">Customer invoices {workbookMode && selectedCurrency === 'AED' ? 'across original invoice currencies' : `in ${currencyLabel(selectedCurrency)}`}. Home amounts are recorded in {homeCurrency}. Grand total includes all {count === null ? 'matching' : count} filtered invoices, across every page.</caption>
          ...
          <tbody>{rows.length > 0 ? rows.map(row => {
            const route = receivableInvoiceRoute(row);
            ...
            return <tr key={row.id || row.invoice_number}>{COLUMNS.map(column => {
              ...
              const title = column.key === 'actual_payment_received' && financeNumber(raw) === null ? workbookMode ? 'Payment not recorded in the source workbook.' : 'Payment not recorded; treated as zero when calculating the remaining balance.' : column.type === 'money' && workbookMode ? `${column.key === 'amount_home' ? homeCurrency : (column.key === 'actual_payment_received' ? row.actual_payment_currency : row.currency) || 'Currency not recorded'} ${value}` : value;
          ...
          {readable && <tfoot><tr><th scope="row" colSpan={5} className="ar-customer-register-grand-total" title={workbookMode ? data.definitions?.totals : `Grand total in ${currencyLabel(selectedCurrency)}; AED amounts in ${homeCurrency}`}>Grand total</th><RegisterTotal metric={data.totals.amount} descriptionId={noteId} /><RegisterTotal metric={data.totals.amount_home} descriptionId={noteId} /><td colSpan={6} /><RegisterTotal metric={data.totals.actual_payment_received} descriptionId={noteId} /><td /></tr></tfoot>}
        </table>
      </div>
      <footer className="ar-customer-register-footer"><p id={noteId}>{partial && (workbookMode ? '* Some totals are unavailable or partial; see column details. ' : '* Recorded subtotal; missing invoice amounts are excluded. ')}{workbookMode ? data.definitions?.totals || 'Grand totals cover all filtered source rows. Original currency totals remain separate; Inv Amt. (AED) is the recorded AED subtotal.' : 'Grand total includes all filtered invoices. Balance = Invoice Amount − Actual Payment Received; blank payments count as zero.'}</p>
...
CustomerInvoicesSection.propTypes = { currency: PropTypes.string, company: PropTypes.string, asOf: PropTypes.string, paymentStatus: PropTypes.string, onPaymentStatusChange: PropTypes.func, sectionRef: PropTypes.object, refreshKey: PropTypes.oneOfType([PropTypes.string, PropTypes.number]), enabled: PropTypes.bool, dataScope: PropTypes.oneOf(['finance', 'executive']), onSnapshotChange: PropTypes.func };
