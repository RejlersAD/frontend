...
import { RECEIVABLE_AGES, RECEIVABLE_KPIS, receivableAlert, receivableChartRows, receivableCollectionRoute, receivableCustomerRows, receivableDate, receivableMetricNote, receivableMoney, receivableNumber, receivableRawValue, receivableReadable, receivablesCsv, receivablesToday, receivableValue } from './financeReceivablesPresentation';
import { CustomerBalancesChart, ExposureDonut, AgeingComparisonChart, OverdueTrendChart, PaymentHistoryChart } from './ReceivablesDashboardCharts';
...
function CustomerTable({ rows, totals, total, currency, allCount, collectionRoute }) {
  ...
  return <div className="ar-table-scroll" tabIndex={0} role="region" aria-label="Customer ageing summary"><table className="ar-table ar-ageing-table" data-table-typography="preserve"><caption className="sr-only">{rows.length} of {allCount} customers. Grand total includes all customers. Amounts in {currency}; asterisks indicate recorded subtotals.</caption>
    ...
    <tbody>{rows.map(row => <tr key={row.customer}>
      <th scope="row">{row.account && row.account !== 'Customer not recorded' ? <Link to={collectionRoute(row.account)} title={row.customer}>{row.customer}</Link> : row.customer}</th>
...
export default function FinanceCommandCenter() {
  ...
  const collectionRoute = useCallback(account => receivableCollectionRoute({ currency, company: filters.company }, account), [currency, filters.company]);
  ...
  return <div className="finance-command-center ar-dashboard" aria-busy={loading}>
    <header className="ar-page-header"><div className="ar-page-title"><h1>Accounts Receivable</h1><p>Monitor collections, ageing and customer exposure</p></div>
      <div className="ar-filters" role="group" aria-label="Receivables filters">
        <label className="ar-filter ar-entity"><span>Entity</span><div><select aria-label="Entity" title="Company recorded on the invoice" value={filters.company} onChange={event => change('company', event.target.value)}><option value="">All entities</option>{options.companies.map(option => { const value = typeof option === 'string' ? option : option.value; return <option key={value} value={value}>{value}</option>; })}</select><ChevronDownIcon aria-hidden="true" /></div></label>
