...
import { RECEIVABLE_AGES, RECEIVABLE_DEFINITIONS, RECEIVABLE_KPIS, receivableAlert, receivableAmountBasis, receivableChartRows, receivableCollectionRoute, receivableCustomer, receivableCustomerRows, receivableDate, receivableInvoiceRoute, receivableMetricNote, receivableMoney, receivableNumber, receivableRawValue, receivableReadable, receivablesCsv, receivablesToday, receivableValue } from './financeReceivablesPresentation';
...
function CustomerTable({ rows, totals, total, currency, allCount, collectionRoute, onCustomerReview }) {
  ...
  return <div className="ar-table-scroll" tabIndex={0} role="region" aria-label="Customer ageing summary"><table className="ar-table ar-ageing-table" data-table-typography="preserve"><caption className="sr-only">{rows.length} of {allCount} customers. Grand total includes all customers. Amounts in {currency}; asterisks indicate recorded subtotals.</caption>
    ...
    <tbody>{rows.map(row => <tr key={row.company}>
      <th scope="row">{row.company ? <>{onCustomerReview ? <button type="button" className="ar-text-button" onClick={() => onCustomerReview(row.company)}>{row.customer}</button> : <Link to={collectionRoute(row.company)} title={row.customer}>{row.customer}</Link>}</> : row.customer}</th>
...
CustomerTable.propTypes = { rows: PropTypes.array.isRequired, totals: PropTypes.array, total: PropTypes.object, currency: PropTypes.string, allCount: PropTypes.number, collectionRoute: PropTypes.func.isRequired, onCustomerReview: PropTypes.func };
...
function InvoiceTable({ rows, total, currency, count }) {
  return <div className="ar-table-scroll" tabIndex={0} role="region" aria-label="Unpaid invoices requiring action"><table className="ar-table ar-invoice-table" data-table-typography="preserve"><caption className="sr-only">Priority invoices shown; grand total covers all {count ?? ''} open invoices in {currency}. Owners are recorded project managers.</caption>
    ...
    <tbody>{rows.map((row, index) => <tr key={row.id}><th scope="row" title={receivableCustomer(row)}>{receivableCustomer(row)}</th><td>{row.invoice_number}</td><td>{row.due_date ? receivableDate(row.due_date) : 'Not recorded'}</td><td className={row.days_overdue > 0 ? 'ar-overdue-days' : 'ar-future-days'}>{financeNumber(row.days_overdue) ?? '—'}</td><td className="ar-total-heat">{receivableNumber(row.balance)}</td><td title={row.owner || 'Project manager not recorded'}>{row.owner || 'Not recorded'}</td><td>{receivableInvoiceRoute(row) ? <Link className={index === 0 ? 'ar-review-link' : 'ar-invoice-more'} to={receivableInvoiceRoute(row)} aria-label={`Review invoice ${row.invoice_number}`}>{index === 0 ? 'Review' : <EllipsisHorizontalIcon aria-hidden="true" />}</Link> : <span title={`Source workbook row ${row.source_row ?? 'not recorded'}`}>Workbook row{row.source_row ? ` ${row.source_row}` : ''}</span>}</td></tr>)}</tbody>
...
export default function FinanceCommandCenter({ embedded = false, dataScope = 'finance', refreshKey = 0, printing = false, onSnapshotChange }) {
  ...
  const [registerPaymentStatus, setRegisterPaymentStatus] = useState('');
  ...
  const registerScope = JSON.stringify([currency, filters.company, `${refreshKey}:${refresh}`, dataScope, asOf, registerPaymentStatus]);
  const workbookMode = data?.sources?.receivables?.mode === 'workbook';
  const reviewWorkbookInvoices = (status = '', company) => {
    if (company !== undefined) change('company', company);
    setRegisterPaymentStatus(status);
    setDialog(null);
    requestAnimationFrame(() => { registerRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); registerRef.current?.focus({ preventScroll: true }); });
  };
  ...
  const primaryOwner = invoices.find(row => row.payment_status === 'overdue' && row.owner)?.owner;
  ...
  return <div className={`finance-command-center ar-dashboard${embedded ? ' ar-dashboard-embedded' : ''}`} aria-busy={loading}>
    ...
    <div className="ar-analysis-grid">
      ...
      <Panel title="Exposure concentration" className="ar-exposure-panel" info={partial ? 'Shares use recorded balances only; missing balances may change the distribution.' : 'Share of unpaid customer balances in the selected currency.'}>{chartState(<ExposureDonut rows={customers} total={receivableValue(data?.kpis?.unpaid)} currency={currency} partial={partial} />)}</Panel>
    ...
    {unknownDueDates > 0 && <p className="ar-data-note">{unknownDueDates} unpaid {unknownDueDates === 1 ? 'invoice has' : 'invoices have'} no due date and cannot be included in 30+, 60+ or 90+ day ageing. Overdue payment statuses still count towards the overdue amount.</p>}
    ...
    <div className="ar-invoice-overview">
      <Panel title="Paid vs unpaid invoices" className="ar-history-panel" info="Recorded payments and current unpaid balances grouped by invoice issue month. This is not a cash receipts timeline." extra={readable && (workbookMode ? <button type="button" className="ar-text-button" onClick={() => reviewWorkbookInvoices()}>View invoice register<ArrowRightIcon /></button> : <Link className="ar-text-button" to={`/finance/outgoing-invoices?${new URLSearchParams({ queue: 'all', currency, company: filters.company })}`}>View invoice register<ArrowRightIcon /></Link>)}>{chartState(<PaymentHistoryChart rows={charts.payments} currency={currency} partial={paymentsPartial} />)}</Panel>
      <CustomerInvoicesSection sectionRef={registerRef} paymentStatus={registerPaymentStatus} onPaymentStatusChange={workbookMode ? setRegisterPaymentStatus : undefined} currency={currency} company={filters.company} asOf={asOf} refreshKey={`${refreshKey}:${refresh}`} enabled={!loading && readable} dataScope={dataScope} onSnapshotChange={setRegisterSnapshot} />
    </div>
    {dialog && <dialog ref={dialogRef} className="ar-dialog" aria-labelledby="ar-dialog-title" onCancel={() => setDialog(null)} onKeyDown={event => {
      ...
      {dialog === 'customers' ? <CustomerTable rows={customers} totals={data?.ageing} total={data?.kpis?.unpaid} currency={currency} allCount={customers.length} collectionRoute={collectionRoute} onCustomerReview={workbookMode ? company => reviewWorkbookInvoices('', company) : undefined} /> : <><p>{receivableAmountBasis(data)} Customer names, grouping and filters use the invoice COMPANY column.</p>{RECEIVABLE_KPIS.map(([id, label]) => <p key={id}><strong>{label}: </strong>{data?.definitions?.[id] || RECEIVABLE_DEFINITIONS[id]}</p>)}<p>The as-of date sets ageing bands for current recorded balances. Monthly charts group these balances by invoice or due month; they do not reconstruct past ledger balances.</p>{Object.entries(data?.sources || {}).map(([key, source]) => <section className="ar-source" key={key}><h3>{key === 'receivables' ? 'Customer invoice register' : 'Supplier invoice register'}</h3><p><strong>{source.status === 'available' ? 'Connected' : source.status === 'incomplete' ? 'Incomplete balances' : source.status === 'restricted' ? 'Access restricted' : 'Unavailable'}</strong>{source.reason ? ` · ${source.reason}` : ''}</p>{source.mode === 'workbook' && source.file_name && <p>Workbook: {source.file_name}{source.sheet_name ? ` / ${source.sheet_name}` : ''}</p>}{key === 'receivables' && financeCount(data?.kpis?.unpaid?.missing_count) !== null && <p>{data.kpis.unpaid.missing_count} of {unpaidCount ?? 'unknown'} open invoices have missing balances. Recorded amounts are shown as partial subtotals.</p>}{key === 'receivables' && workbookMode && source.status !== 'restricted' ? <button type="button" className="ar-text-button" onClick={() => reviewWorkbookInvoices()}>View source invoices<ArrowRightIcon /></button> : source.route && source.status !== 'restricted' && <Link to={source.route} className="ar-text-button">Open register<ArrowRightIcon /></Link>}</section>)}{!data && <p>Load the dashboard to see source coverage.</p>}<p>Collection ownership uses the recorded project manager. Action deadlines are shown only when a scheduling source is connected.</p></>}
