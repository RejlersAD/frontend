...
const emptyDecision = { loading: false, reloading: false, stale: false, mode: null, reason: '', message: '', error: '' }
...
const ProcurementApprovalPreviewModal = ({ isOpen, type, recordId, onClose, onDecision, approvalStage, canDecide = true, initialAction = 'view' }) => {
  ...
  contextRef.current = `${isOpen}:${type}:${recordId}`
  const decisionAllowed = canDecide && canDecideProcurement(record.data, null, type)
  const needsReload = type === 'pr' && (decision.stale || (record.data && !record.data.updated_at))
  ...
  useEffect(() => {
    ...
    setDecision({ ...emptyDecision, mode: canDecide && initialAction === 'reject' ? 'reject' : null })
    setActivePreview('document')
    setLinkedPoPdf(emptyPdfPreview)
  ...
  }, [isOpen, recordId, type, initialAction, canDecide])
  ...
  const reloadRecord = async () => {
    if (decision.loading || decision.reloading || type !== 'pr') return
    const context = contextRef.current
    setDecision(current => ({ ...current, reloading: true, error: '' }))
    try {
      const { data } = await apiClient.get(`/procurement/requisitions/${recordId}/`, { params: { _fresh: Date.now() }, suppressErrorToast: true })
      if (contextRef.current !== context) return
      if (String(data?.id) !== String(recordId) || !data.updated_at) throw new Error('The current recommendation version could not be confirmed. Please retry.')
      setRecord({ loading: false, data, error: '' })
      setDecision(current => ({ ...current, stale: false, reloading: false, error: '', message: '' }))
    } catch (error) {
      if (contextRef.current === context) setDecision(current => ({ ...current, reloading: false, stale: true, error: error.response?.data?.detail || error.message || 'The latest recommendation could not be loaded.' }))
    }
  }

  const submitDecision = async (action) => {
    if (!decisionAllowed || decision.loading || decision.reloading || needsReload || !record.data || !['approve', 'reject'].includes(action)) return
    ...
    try {
      ...
      const payload = isPurchaseOrder
        ...
        : action === 'approve'
          ? { signature: '', expected_updated_at: record.data.updated_at }
          : { reason, expected_updated_at: record.data.updated_at }
      ...
      setDecision((current) => ({
        ...current,
        loading: false,
        stale: type === 'pr' && requestError.response?.status === 409,
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="approval-record-preview-title" className="flex max-h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/20 bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {record.data && !record.loading && !record.error && (
          <footer className="border-t border-slate-200 bg-white px-4 py-3 sm:px-6">
            ...
            {decision.error && <div role="alert" className="mb-3 flex items-center gap-2 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700"><ExclamationTriangleIcon className="h-4 w-4 flex-none" />{decision.error}</div>}
            {needsReload && <div className="mb-3 text-sm text-amber-900"><p>Reload and review the latest recommendation before recording a decision. Your rejection reason is retained.</p><button type="button" onClick={reloadRecord} disabled={decision.reloading} className="mt-2 font-semibold underline">{decision.reloading ? 'Reloading...' : 'Reload latest version'}</button></div>}
            ...
            {decisionAllowed && !decision.message && (
              <div className="flex flex-wrap items-center justify-end gap-2">
                ...
                <button type="button" onClick={() => decision.mode === 'reject' ? submitDecision('reject') : setDecision((current) => ({ ...current, mode: 'reject', message: '', error: '' }))} disabled={decision.loading || decision.reloading || needsReload} className="inline-flex h-9 items-center gap-2 rounded-md border border-rose-300 bg-white px-4 text-xs font-semibold text-rose-700 transition hover:bg-rose-50 disabled:opacity-40">
                ...
                {decision.mode !== 'reject' && <button type="button" onClick={() => submitDecision('approve')} disabled={decision.loading || decision.reloading || needsReload} className="inline-flex h-9 items-center gap-2 rounded-md bg-emerald-600 px-4 text-xs font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-40"><CheckCircleIcon className="h-4 w-4" />{decision.loading ? 'Approving...' : 'Approve'}</button>}
              </div>
            )}
          </footer>
        )}
...
ProcurementApprovalPreviewModal.propTypes = {
  ...
  approvalStage: PropTypes.string,
  canDecide: PropTypes.bool,
  initialAction: PropTypes.oneOf(['view', 'approve', 'reject']),
}
