    ...
    <main className="enquiry-typography min-h-full w-full min-w-0 bg-[#f5f8fc] p-4 sm:p-5">
      ...
      <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="space-y-4 xl:sticky xl:top-4">
          ...
          {enquiry.approval_required && <Card title="Approval point"><div className="text-xs text-slate-600"><div className="flex justify-between"><span>Handling approval</span><strong className="uppercase">{enquiry.approval_status}</strong></div>{enquiry.approved_by && <div className="mt-1 text-slate-400">{enquiry.approved_by.name} · {formatDate(enquiry.approved_at)}</div>}</div>{enquiry.can_approve === true && enquiry.approval_status === 'pending' && <div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => patch({ approval_status: 'rejected' })} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-bold text-red-600">Reject</button><button onClick={() => patch({ approval_status: 'approved' })} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white">Approve</button></div>}</Card>}
