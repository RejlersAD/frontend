...
export default function EnquiryManagement () {
  ...
  return (
    <main className="enquiry-typography min-h-full w-full min-w-0 bg-[#f5f8fc] p-4 text-slate-800 sm:p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[950px] text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && !error && items.map((row) => <tr key={row.id} onClick={() => navigate(`/admin/enquiries/${row.id}`)} className="cursor-pointer transition hover:bg-blue-50/60"><Td className="font-mono font-semibold text-blue-700">{row.reference}</Td><Td><div className="font-semibold text-slate-800">{row.name}</div><div className="text-[11px] text-slate-400">{row.company || row.email}</div></Td><Td className="max-w-[280px] truncate font-medium">{row.subject}</Td><Td>{row.department || 'Unrouted'}</Td><Td>{row.assigned_to?.name || 'Unassigned'}</Td><Td><Priority value={row.urgency} /></Td><Td><Status value={row.status} /></Td><Td className={`enquiry-mono text-[11px] ${row.is_overdue ? 'font-bold text-red-600' : ''}`}>{date(row.due_at, true)}</Td><Td><ChevronRightIcon className="h-4 w-4 text-slate-400" /></Td></tr>)}
            </tbody>
          </table>
        </div>
        <div className="flex items-center justify-between border-t bg-slate-50 px-4 py-3 text-xs text-slate-500"><span className="enquiry-mono">{count ? `${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, count)} of ${count}` : '0 records'}</span><div className="flex items-center gap-2"><button disabled={page === 1} onClick={() => setPage((value) => value - 1)} className="rounded border bg-white px-3 py-1.5 disabled:opacity-40">Previous</button><span className="enquiry-mono">{page} / {totalPages}</span><button disabled={page === totalPages} onClick={() => setPage((value) => value + 1)} className="rounded border bg-white px-3 py-1.5 disabled:opacity-40">Next</button></div></div>
      </section>
    </main>
  )
}
...
const Metric = ({ label: text, value, note, tone }) => <div><div className="text-[10px] text-slate-500">{text}</div><div className="enquiry-mono mt-1 text-3xl font-bold tracking-tight text-slate-800">{value}</div><span className={`mt-2 inline-block rounded-sm px-1.5 py-0.5 text-[9px] font-bold ${tone === 'red' ? 'bg-red-50 text-red-700' : tone === 'blue' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'}`}>{note}</span></div>
const Signal = ({ value, label: text, tone }) => <div className={`rounded-sm py-5 text-center ${tone === 'red' ? 'bg-red-50 text-red-700' : tone === 'amber' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}><div className="enquiry-mono text-2xl font-black">{value}</div><div className="mt-1 text-[10px] font-bold">{text}</div></div>
const Gauge = ({ value }) => { const safe = Math.min(100, Math.max(0, value)); return <div className="relative h-24 w-24 shrink-0"><svg viewBox="0 0 100 100" className="h-full w-full -rotate-90"><circle cx="50" cy="50" r="39" fill="none" stroke="#e5e7eb" strokeWidth="9" strokeDasharray="196 50" strokeLinecap="round"/><circle cx="50" cy="50" r="39" fill="none" stroke="#2563eb" strokeWidth="9" strokeDasharray={`${safe * 1.96} 246`} strokeLinecap="round"/></svg><div className="absolute inset-0 grid place-content-center text-center"><strong className="enquiry-mono text-lg">{safe}%</strong><span className="text-[9px] text-slate-400">Target 95%</span></div></div> }
...
const DepartmentMix = ({ rows, total }) => { const top = rows.slice(0, 5); let cursor = 0; const segments = top.map((row, index) => { const start = cursor; cursor += total ? row.count / total * 100 : 0; return `${MIX_COLORS[index]} ${start}% ${cursor}%` }); if (cursor < 100) segments.push(`#e2e8f0 ${cursor}% 100%`); return <div className="flex items-center gap-6"><div style={{ background: `conic-gradient(${segments.join(',')})` }} className="relative h-28 w-28 shrink-0 rounded-full"><div className="absolute inset-6 grid place-content-center rounded-full bg-white text-center"><strong className="enquiry-mono text-lg">{number(total)}</strong><span className="text-[8px] text-slate-400">Requests</span></div></div><div className="min-w-0 flex-1 space-y-2">{top.map((row, index) => <div key={row.name} className="flex items-start gap-2 text-[10px]"><span style={{ backgroundColor: MIX_COLORS[index] }} className="mt-1 h-2 w-2 shrink-0 rounded-full"/><span className="min-w-0 flex-1 truncate font-semibold">{row.name}</span><span className="enquiry-mono text-slate-400">{row.count}</span></div>)}</div></div> }
