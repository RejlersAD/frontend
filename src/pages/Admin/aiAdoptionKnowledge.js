...
export const adoptionKnowledge = [
  {
    id: 'overview', title: 'Overview', purpose: 'Monitor RADAI platform activity, including page visits. AI means RADAI here; model providers are API integrations inside RADAI, not separate employee tools.',
    steps: ['Use Live user activity to search every user in your reporting scope, including users with no recent observations.', 'Filter by Processing, Recently active, Needs workflow check or No recent signal. Filter by module and sort by user, state or last signal across all results. Select a row or its action button for that user’s timeline.', 'Pause automatic updates when reviewing a row. User activity details has Overview, Activity log and AI requests tabs; previous/next moves through filtered users, including across pages. View activity history retrieves up to 100 recent events. Use the contribution register below for period-based request evidence.'],
    metrics: ['The live list refreshes every 15 seconds while visible. Its rolling 24-hour window is independent of the report-period controls.', 'Recent activity means a signal in the last five minutes. AI processing means a server workflow is marked running and began within six hours. Older running jobs need checking.', 'AI calls count server-observed SDK operations linked to workflows. Submitted activity events are separate and may include browsing. Module filtering scopes signals and counts to that module; summary metrics reflect search and module before the state filter. Outstanding workflow checks can include running jobs older than 24 hours.'],
  ...
  },
  {
    ...
    steps: ['Choose the UTC award month. Select up to 20 eligible employees using row checkboxes or Select top 20; the selected employees are ranked by activity score and ranks 1?10 are recognized. Fewer eligible employees means fewer places.', 'Treat the open month as a provisional preview. Publication requires a completed month.', 'A Super Administrator reviews the server-calculated shortlist and top 10, supplies a reason and confirms publication. The selected IDs and scores are saved in the award snapshot.', 'Inspect published snapshots without recalculating their original methodology.'],
    metrics: ['Active accounts qualify with any recorded RADAI activity in the selected month, including page visits. A model API call is not required. Current-month candidates are provisional until reviewed after month end.', 'Monthly ranking uses the methodology displayed for that month. It is separate from the workforce 40/30/30 engagement indicator.', 'Published results retain the reviewed period, score, reason and reviewer information.'],
    limits: ['A leading candidate is not yet an awarded champion.', 'RADAI activity includes page visits and is not employee performance or demonstrated business value.', 'Historical browser-only honorary entries are not verified monthly awards. The old URL redirects to this workspace.'],
  ...
  },
  {
    ...
    metrics: ['Page views, other activity and AI requests are separate signals.', 'Activity without provider calls still counts as RADAI adoption and monthly champion engagement.', 'Request success is a technical result, not a business outcome.'],
