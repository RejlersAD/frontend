...
export default function ProjectDetailsHeader({ projects, selectedProject, selectedProjectId, onSelectProject, loading, error,
  ...
  return <header className="pd-header" aria-label="Project details">
    <div className="pd-heading-content">
      <div className="pd-location"><button type="button" onClick={() => onNavigate('/projects')}><ArrowLeft size={16} />Back to projects</button><span className="pd-location-divider" aria-hidden="true" />
        <nav aria-label="Breadcrumb"><span>Project Control</span><span aria-hidden="true">/</span><button type="button" onClick={() => onNavigate('/projects')}>Portfolio</button><span aria-hidden="true">/</span><span aria-current="page">{project?.code || 'Project details'}</span></nav>
      </div>
      ...
      <div className="pd-header-actions">
        ...
        </div></details> : <button type="button" className="pd-button pd-primary" onClick={primaryAction} disabled={primaryDisabled}><PrimaryIcon size={16} />{primaryLabel}</button>}
        <button type="button" className="pd-button" onClick={() => onOpenDialog('actions')} disabled={!model || busy}><AlertCircle size={17} />{actionCount ? `Resolve ${actionCount} ${actionCount === 1 ? 'issue' : 'issues'}` : 'Review issues'}</button>
        <button type="button" className="pd-button" aria-label={exportLabel} onClick={onExport} disabled={exportDisabled} title={exportLabel === 'Export' ? 'Print or save the project report as PDF' : exportLabel}><Download size={18} />Export</button>
      ...
      {project && <div className="pd-project-facts">
