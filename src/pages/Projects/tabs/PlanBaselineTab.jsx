...
export default function PlanBaselineTab({ project, schedulePerformance, scheduleMode = 'management', onScheduleMode, onSelectBaseline, onSelectVersion }) {
  ...
  return <div className="sp-workspace">
    <div className="sp-toolbar"><div className="sp-mode-switch" role="group" aria-label="Schedule workspace mode">{['management', 'planner'].map(mode => <button type="button" key={mode} aria-pressed={scheduleMode === mode} onClick={() => onScheduleMode(mode)}>{labelOf(mode)}</button>)}</div>
    ...
    {scheduleMode === 'planner' ? <div className="sp-planner-container"><PlanningPackagePage key={project.id} embedded enterpriseProject={project} /></div> : <>
