...
export default function MyEnquiryDetail () {
  ...
  return <main className="w-full min-w-0 bg-slate-50 px-4 py-5 sm:px-6">
    ...
    <header className="rounded-2xl bg-gradient-to-r from-slate-950 to-blue-900 p-6 text-white"><div className="font-mono text-xs text-blue-200">{ticket.reference}</div><div className="mt-1 flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-bold">{ticket.subject}</h1><p className="mt-1 text-xs text-blue-100">{ticket.department || 'Routing pending'} · Submitted {date(ticket.created_at)}</p></div><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-bold">{label(ticket.status)}</span></div></header>
