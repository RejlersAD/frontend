...
import { ArrowDownTrayIcon, ArrowPathIcon, ChevronDownIcon, EllipsisVerticalIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
...
import { financialMetric, financialReport } from './financialPresentation';
...
export default function ExecutiveDashboard() {
  ...
  const exportSnapshot = () => {
    const snapshot = activeTab === 'financial' ? {
      ...
      revenue: financialMetric(financialReport(report), 'revenue_ytd', financialCurrency),
      project_register: portfolio.register,
    } : report;
  ...
  const financialCurrencies = [...new Set([financialCurrency, ...(financialSnapshot?.receivables?.filters?.currencies || [])])].sort();
      ...
      <div className="cc-title-row"><div className="cc-title-block"><div className="cc-breadcrumb"><span>Executive</span> / {TABS.find(tab => tab.id === activeTab)?.label}</div><h1>{isFinancial ? 'Financial Performance' : isPortfolio ? 'Project Portfolio' : isCommercial ? 'Commercial Pipeline' : isWorkforce ? 'Workforce' : isRisk ? 'Risk & Compliance' : 'Executive Command Center'}</h1><p>{isFinancial ? 'Revenue, profitability, cash and forecast performance' : isPortfolio ? 'Portfolio value, delivery confidence, margin exposure and interventions.' : isCommercial ? 'Pipeline value, win confidence, client exposure and delivery demand.' : isWorkforce ? 'Capacity, utilization, critical skills and workforce risk.' : isRisk ? 'Enterprise risk, QHSE performance, obligations and assurance.' : 'Company performance, portfolio risk and decisions requiring attention.'}</p></div>
        <div className="cc-header-right"><div className="cc-toolbar cc-screen-only">
          {isFinancial ? <>
            ...
            <select className="cc-button ef-currency-select" aria-label="Financial reporting currency" title="Currency for current receivables. Workbook totals retain their original currencies." value={financialCurrency} disabled={financialBusy} onChange={event => setFinancialCurrency(event.target.value)}>{financialCurrencies.map(value => <option key={value} value={value}>{value === 'UNSPECIFIED' ? 'Unspecified' : value}</option>)}</select>
            <button className="cc-button cc-scope-button" onClick={() => explainScope('period')} aria-label="Financial date scope" aria-haspopup="dialog">All periods<ChevronDownIcon /></button>
