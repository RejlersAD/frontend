...
function RequisitionLinkStatus({ record, onEdit, onEditDocument, canEdit = true }) {
...
function OrderDetails({ record, loading, error, onOpen, onEdit, onPdf, onAcknowledge, onRetry, onIssue, onPreviewDocument, onEditDocument, onDeleteDocument, pdfBusy, canEdit, canDelete }) {
  ...
  return <aside className="prw-panel prw-details" aria-label="Purchase order details" aria-busy={loading} tabIndex={-1}>
    ...
    <div className="prw-detail-body" tabIndex={0} role="region" aria-label="Purchase order detail content">
      ...
      <div className="prw-detail-footer"><button className="prw-link" type="button" onClick={() => navigate('/procurement/receipts')}><Truck size={14} />Open receipts <ArrowRight size={13} /></button>{!issued && approved && <button type="button" className="prw-button prw-primary" onClick={() => onIssue(raw)} disabled={!canEdit || loading || Boolean(error)}><Send size={14} />Issue order</button>}{canEdit && record.status !== 'completed' && <button type="button" className="prw-button" onClick={() => onEdit(raw)}>Edit order</button>}</div>
    </div>
  </aside>
}

export default function ProcurementRegister({ orders, loading, error, pendingUploadError, currentUserId, requisitionCount, onRefresh, onCreate, onImportPdf, onImportExcel, onPreviewDocument, onEditDocument, onDeleteDocument, onExport, onOpen, onEdit, onDelete, onIssue, onPdf, onAcknowledge, pdfBusy, canCreate = true, canEdit = true, canDelete = true }) {
