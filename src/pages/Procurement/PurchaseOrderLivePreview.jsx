...
const FinalSignatory = ({ name, designation, signedDate }) => <div className="max-w-[250px]"><p className="text-[10px] font-bold">{text(nameOnly(name), FINAL_APPROVER)}</p><p className="whitespace-pre-line">{text(designation, FINAL_APPROVER_TITLE)}</p><p>{FINAL_APPROVER_COMPANY}</p><p className="mt-1"><b>Date:</b> {signedDate ? dateTime(signedDate).split(',')[0] : ''}</p></div>;
FinalSignatory.propTypes = { name: PropTypes.string, designation: PropTypes.string, signedDate: PropTypes.string };

const DocumentHeader = ({ data }) => <header className="flex items-start justify-between px-1 pb-5"><div className="text-[#3275b6]"><h1 className="text-[12px] font-black uppercase tracking-wide">Purchase Order</h1><p className="mt-0.5 text-[10px] font-black">{text(data.po_number, 'PO NUMBER PENDING')}</p><p className="text-[7px] text-slate-500">{text(data.form_note, '(PO no. to be used in all documents)')}</p><p className="mt-2 text-[10px] font-bold">{date(data.po_date)}</p></div><div className="w-[100px] text-right"><img src={PROCUREMENT_DOCUMENT_BRANDING.logo.path} alt={PROCUREMENT_DOCUMENT_BRANDING.logo.alt} className="ml-auto h-auto w-full object-contain" /><p className="mt-1 text-[9px] font-bold leading-[10px] tracking-tight text-[#3275b6]">HOME OF THE<br />LEARNING MINDS</p></div></header>;
...
const Page = ({ data, page, children, finalApproval, showPageStamp = true }) => <section className="po-template-page relative mx-auto flex aspect-[210/297] w-full max-w-[560px] flex-col overflow-hidden border border-slate-400 bg-white px-10 py-6 text-[10px] leading-[1.3] text-slate-700 shadow-xl">{showPageStamp && <ApprovalStamp approval={finalApproval} />}<div className="relative z-[1]"><DocumentHeader data={data} /></div><div className="relative z-[1] flex-1">{children}</div><div className="relative z-[1]"><DocumentFooter data={data} page={page} /></div></section>;
...
const Pair = ({ label, value, strong = false, blank = false }) => <div className="grid grid-cols-[82px_1fr] gap-2"><b className="text-slate-600">{label}:</b><span className={`whitespace-pre-line ${strong ? 'font-bold text-slate-800' : ''}`}>{text(value, blank ? '' : '—')}</span></div>;
Pair.propTypes = { label: PropTypes.string.isRequired, value: PropTypes.node, strong: PropTypes.bool, blank: PropTypes.bool };
  ...
  return <div className={documentOnly ? 'po-template-document' : 'h-full overflow-y-auto bg-slate-200 p-4'}>
    ...
    <div className={documentOnly ? 'po-template-pages' : 'space-y-5'}>
      <Page data={formData} page={1} finalApproval={finalApproval} showPageStamp={false}>
        ...
        <div className="grid min-h-[181px] grid-cols-2 gap-x-8"><div className="space-y-2"><Pair label="Seller" value={vendor?.name} /><Pair label="Seller Address" value={formData.seller_address || vendor?.address || vendor?.country} /><Pair label="Invoicing Address" value={DEFAULT_INVOICE_ADDRESS} /></div><div className="space-y-2"><Pair label="Seller Reference" value={formData.seller_reference} /><Pair label="Quote Ref." value={formData.quote_ref} /><Pair label="License No." value={formData.seller_license_no} /><Pair label="Buyer Reference" value={buyerReference} /></div></div>
        <div className="mt-3 grid min-h-[72px] grid-cols-2 gap-x-8"><div className="space-y-2"><Pair label="Payment Terms" value={formData.payment_terms} /><Pair label="Payment Mode" value={formData.payment_mode} /><Pair label="Project" value={project} strong /></div><div className="space-y-2"><Pair label="Delivery terms" value={formData.delivery_terms} />{formData.contact_persons?.delivery_date_type === 'start' ? <><Pair label="Start date" value={date(formData.start_date)} /><Pair label="End date" value={date(formData.end_date)} /></> : <Pair label="Delivery date" value={date(formData.expected_delivery)} />}<Pair label="Marking" value={formData.marking || formData.po_number} strong /></div></div>
        <div className="mt-3 grid min-h-[53px] grid-cols-[1.15fr_1fr] gap-6 border-y-2 border-slate-600 py-2">
