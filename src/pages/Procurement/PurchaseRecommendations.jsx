...
import { Field, KeyValues, Menu, isRegisterControl } from './ProcurementRegister'
...
function RecommendationDetails({ panelRef, record, loading, error, onRetry, onOpen, onEdit, onConvert, onPdf, onWord, onAttachPdf, canModify, canConvert, pdfBusyId, wordBusyId, converting }) {
  ...
  return <aside ref={panelRef} tabIndex={-1} className="prw-panel prw-details" aria-label="Recommendation details" aria-busy={loading}>
    ...
    <div className="prw-detail-body" tabIndex={0} role="region" aria-label="Recommendation decision and approvals">
      ...
      <div className={`prr-conversion-footer${record.readyForPO || record.linkedPoId ? ' is-ready' : ''}`}><ShoppingCart size={23} /><div><strong>{record.linkedPoId ? 'Purchase order created' : record.status === 'converted' ? 'Conversion recorded' : record.readyForPO ? 'Ready to create purchase order' : approved ? 'Review conversion requirements' : 'Complete recommendation approval'}</strong><p>{record.linkedPoId ? 'Open the linked order to track supplier delivery.' : record.status === 'converted' ? 'The linked order is not available in this record.' : record.readyForPO ? 'Values copy into a new PO draft after final checks.' : record.status === 'approved' ? record.nextStep : 'Approval is required before creating a PO.'}</p></div><div className="prr-conversion-actions">{canModify(raw) && <button type="button" className="prw-button" disabled={loading || Boolean(error)} onClick={() => onEdit(raw)}>{record.status === 'rejected' ? 'Edit and resubmit' : 'Edit recommendation'}</button>}{record.linkedPoId ? <Link className="prw-button prw-primary" to={`/procurement/orders/${record.linkedPoId}`}>View purchase order</Link> : convertible ? <button type="button" className="prw-button prw-primary" disabled={loading || Boolean(error) || converting} onClick={() => onConvert(raw)}>{converting ? 'Creating…' : record.readyForPO ? 'Create purchase order' : 'Validate & create PO'}</button> : null}</div></div>
  </aside>
}

export default function PurchaseRecommendations({ requisitions = [], loading = false, error = null, currentUserId, orderCount, onRefresh, onCreate, onImportPdf, onImportExcel, onExport, onOpen, onEdit, onDelete, onConvert, onPdf, onWord, onAttachPdf, onApproveSelected, canLinkPurchaseOrder = false, canUploadPurchaseOrder = false, canModify = deny, canDelete = deny, canConvert = deny, canApprove = deny, pdfBusyId, wordBusyId, batchBusy = false, canCreate = true }) {
  ...
  return <section className="procurement-register-workspace purchase-recommendations-workspace" aria-label="Purchase recommendations workspace">
    ...
    <RecommendationDetails panelRef={previewPanelRef} record={detailed} loading={currentDetail.loading} error={currentDetail.error} onRetry={() => setDetailRevision(value => value + 1)} onOpen={onOpen} onEdit={onEdit} onConvert={convert} onPdf={onPdf} onWord={onWord} wordBusyId={wordBusyId} onAttachPdf={onAttachPdf} canLinkPurchaseOrder={canLinkPurchaseOrder} canModify={canModify} canConvert={canConvert} pdfBusyId={pdfBusyId} converting={converting} />
    </div>
