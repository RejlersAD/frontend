...
import { AlertTriangle, ArrowRight, BarChart3, Check, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Download, FileCheck2, FileText, Filter, Home, Plus, RefreshCw, Search, ShieldCheck, ShoppingCart, Unlink, Users, X, XCircle } from 'lucide-react'
...
function RecommendationStatus({ record }) {
  ...
  return <span className={`prw-status prw-${record.tone}`}><Icon size={16} /><span>{record.statusLabel}<small className="prw-secondary" title="Age since this recommendation was created">{record.ageDays == null ? 'Age unavailable' : `${record.ageDays}d`}</small></span></span>
}

function RecommendationDetails({ record, loading, error, onRetry, onOpen, onEdit, onConvert, onPdf, onAttachPdf, canModify, canConvert, pdfBusyId, converting }) {
  if (!record) return <aside className="prw-panel prw-details" aria-label="Recommendation details"><div className="prw-panel-header"><h2>Recommendation details</h2></div><div className="prw-empty"><FileText size={32} /><h3>Select a recommendation</h3><p>Choose a record to review its sourcing decision, approvals and next step.</p></div></aside>
  ...
  return <aside className="prw-panel prw-details" aria-label="Recommendation details" aria-busy={loading}>
    <div className="prw-panel-header"><h2>Recommendation details</h2><Menu label="More recommendation detail actions"><button type="button" role="menuitem" onClick={() => onOpen(record.id)}>Open recommendation</button>{canModify(raw) && <button type="button" role="menuitem" onClick={() => onEdit(raw)}>Edit recommendation</button>}{onAttachPdf && canModify(raw) && <button type="button" role="menuitem" onClick={() => onAttachPdf(raw)}>Attach signed PDF</button>}{record.status === 'approved' && <button type="button" role="menuitem" disabled={pdfBusyId === record.id || loading} onClick={() => onPdf(raw)}>Preview signed PDF</button>}</Menu></div>
    <div className="prw-detail-body" tabIndex={0} role="region" aria-label="Recommendation decision and approvals">
      ...
      <div className="prw-detail-heading"><div className="prr-detail-reference"><span className="prw-secondary">{record.number}</span><span className={`prw-badge prw-${record.tone}`}><CheckCircle2 size={11} />{record.statusLabel}</span>{record.priority && <span className="prw-badge prw-blue"><ShieldCheck size={11} />{record.priority} priority</span>}</div><h3>{record.title}</h3></div>
      ...
      <section className="prr-approval-history"><h4>Approval history</h4><div className="prr-history-content">
        {history.length ? <ol>{history.map((step, index) => <li className="prr-history-entry" key={`${index}-${step.label}`} title={`${step.label}: ${step.assignee}`}>
          ...
          <span>{step.label}</span><strong className={isComplete(step.status) ? 'prw-green' : step.status === 'rejected' ? 'prw-red' : 'prw-secondary'}>{step.status.replaceAll('_', ' ')}</strong><time>{date(step.date)}</time>
      ...
      <div className={`prr-conversion-footer${record.readyForPO || record.linkedPoId ? ' is-ready' : ''}`}><ShoppingCart size={23} /><div><strong>{record.linkedPoId ? 'Purchase order created' : record.status === 'converted' ? 'Conversion recorded' : record.readyForPO ? 'Ready to create purchase order' : approved ? 'Review conversion requirements' : 'Complete recommendation approval'}</strong><p>{record.linkedPoId ? 'Open the linked order to track supplier delivery.' : record.status === 'converted' ? 'The linked order is not available in this record.' : record.readyForPO ? 'Values copy into a new PO draft after final checks.' : record.status === 'approved' ? record.nextStep : 'Approval is required before creating a PO.'}</p></div><div className="prr-conversion-actions"><button type="button" className="prw-button" onClick={() => onOpen(record.id)}>Open recommendation</button>{record.linkedPoId ? <Link className="prw-button prw-primary" to={`/procurement/orders/${record.linkedPoId}`}>View purchase order</Link> : convertible ? <button type="button" className="prw-button prw-primary" disabled={loading || Boolean(error) || converting} onClick={() => onConvert(raw)}>{converting ? 'Creating…' : record.readyForPO ? 'Create purchase order' : 'Validate & create PO'}</button> : null}</div></div>
  </aside>
}

export default function PurchaseRecommendations({ requisitions = [], loading = false, error = null, currentUserId, orderCount, onRefresh, onCreate, onImportPdf, onImportExcel, onExport, onOpen, onEdit, onDelete, onConvert, onPdf, onAttachPdf, onApproveSelected, canLinkPurchaseOrder = false, canModify = deny, canDelete = deny, canConvert = deny, canApprove = deny, pdfBusyId, batchBusy = false }) {
  const [linkRecord, setLinkRecord] = useState(null)
  const linkDialog = useRef(null)
  useEffect(() => {
    if (linkRecord) linkDialog.current?.showModal()
  ...
  const primaryLabel = row => row.linkedPoId ? 'View PO' : row.status === 'approved' && canConvert(row.raw) ? row.readyForPO ? 'Create PO' : 'Review' : ['draft', 'rejected'].includes(row.status) && canModify(row.raw) ? row.status === 'rejected' ? 'Revise' : 'Edit' : row.awaitingApproval ? 'Review' : 'Open'
  const primary = row => { if (row.status === 'approved' && row.readyForPO && canConvert(row.raw)) convert(row.raw); else if (['draft', 'rejected'].includes(row.status) && canModify(row.raw)) onEdit(row.raw); else onOpen(row.id) }
  ...
  return <section className="procurement-register-workspace purchase-recommendations-workspace" aria-label="Purchase recommendations workspace">
    ...
    {loading ? <tr><td colSpan={bulk ? 11 : 10}><div className="prw-loading" role="status"><RefreshCw size={20} />Loading recommendations…</div></td></tr> : !visible.length ? <tr><td colSpan={bulk ? 11 : 10}><div className="prw-empty"><FileText size={32} /><h3>{error ? 'Register unavailable' : records.length ? 'No matching recommendations' : 'No recommendations yet'}</h3><p>{error ? 'Refresh to try loading the register again.' : records.length ? 'Clear or adjust your filters to find a recommendation.' : 'Create a recommendation or import your existing register.'}</p>{!error && <button type="button" className="prw-button" onClick={records.length ? reset : onCreate}>{records.length ? 'Clear filters' : 'New recommendation'}</button>}</div></td></tr> : visible.map(row => <tr key={row.id} className={row.id === activeId ? 'prw-selected-row' : ''} onClick={() => setSelectedId(row.id)}>{bulk && <td><input type="checkbox" aria-label={`Select for bulk ${row.number}`} checked={checkedIds.includes(String(row.id))} onClick={event => event.stopPropagation()} onChange={() => toggleChecked(row.id)} /></td>}<td><RecommendationStatus record={row} /></td><td><button type="button" className="prw-link prw-number" aria-label={`Select ${row.number}`} aria-pressed={row.id === activeId} onClick={() => setSelectedId(row.id)}>{row.number}</button><RecommendationPOLink record={row} canLink={canLinkPurchaseOrder} onLink={setLinkRecord} /></td><td title={row.title}><span className="prr-cell-title">{row.title}</span></td><td title={row.project}><span className="prr-cell-project">{row.project}</span></td><td title={row.supplier}><span className="prr-cell-supplier">{row.supplier}</span></td><td className="prw-money">{money(row.amount, row.currency)}</td><td>{row.requester}</td><td>{row.currentOwner}</td><td className={row.approvalSummary === 'overdue' || row.hasException ? 'prw-amber' : ''}>{row.nextStep}</td><td><div className="prw-row-actions">{row.linkedPoId ? <Link className="prw-button" to={`/procurement/orders/${row.linkedPoId}`}>View PO</Link> : <button type="button" className={`prw-button${row.readyForPO && canConvert(row.raw) ? ' prw-primary' : ''}`} disabled={converting} onClick={event => { event.stopPropagation(); setSelectedId(row.id); primary(row) }}>{primaryLabel(row)}</button>}<Menu label={`Actions for ${row.number}`}><button type="button" role="menuitem" onClick={() => onOpen(row.id)}>Open recommendation</button>{canModify(row.raw) && <button type="button" role="menuitem" onClick={() => onEdit(row.raw)}>Edit recommendation</button>}{onAttachPdf && canModify(row.raw) && <button type="button" role="menuitem" onClick={() => onAttachPdf(row.raw)}>Attach signed PDF</button>}{row.status === 'approved' && <button type="button" role="menuitem" disabled={pdfBusyId === row.id} onClick={() => onPdf(row.raw)}>Preview signed PDF</button>}{canDelete(row.raw) && <button type="button" role="menuitem" onClick={() => onDelete(row.raw)}>Delete recommendation</button>}</Menu></div></td></tr>)}
    ...
    <RecommendationDetails record={detailed} loading={currentDetail.loading} error={currentDetail.error} onRetry={() => setDetailRevision(value => value + 1)} onOpen={onOpen} onEdit={onEdit} onConvert={convert} onPdf={onPdf} onAttachPdf={onAttachPdf} canLinkPurchaseOrder={canLinkPurchaseOrder} canModify={canModify} canConvert={canConvert} pdfBusyId={pdfBusyId} converting={converting} />
    </div>
    <dialog ref={linkDialog} className="prr-link-dialog" aria-labelledby="prr-link-title" onCancel={() => setLinkRecord(null)}>
      ...
      {linkRecord && <PurchaseOrderLinkReview key={linkRecord.id} requisitionId={linkRecord.id} poLink={{ status: 'not_found', manual_link_required: true, message: `Select the purchase order for ${linkRecord.number}.` }} canLink={canLinkPurchaseOrder} onLinked={() => { setLinkRecord(null); setDetailRevision(value => value + 1); onRefresh?.() }} />}
    </dialog>
  </section>
}
