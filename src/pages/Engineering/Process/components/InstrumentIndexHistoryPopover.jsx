...
import {
  Gauge, Loader2, RefreshCw, Trash2, CheckCircle2, X, Layers,
} from 'lucide-react'

import {
  listInstrumentIndexes, activateInstrumentIndex, deleteInstrumentIndex,
...
// Amber accent (matches InstrumentIndexDropzone)
...
/**
 * Compact history popover for uploaded Instrument Indexes.
 *
 * Props:
...
export default function InstrumentIndexHistoryPopover({ activeInstrumentIndex, onChange }) {
  ...
  const onActivate = async (id) => {
    try {
      await activateInstrumentIndex(id)
      await refresh()
      onChange?.()
    } catch (err) {
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative' }}>
      ...
      >
        <Layers size={12} /> Manage instrument indexes
      </button>
          ...
          <div style={{ padding: 8 }}>
            {history.length === 0 && !loading && (
              <div style={{ padding: 12, textAlign: 'center', color: THEME_MUTED, fontSize: 12 }}>
                No instrument indexes uploaded yet.
              </div>
            )}
            <div style={{ display: 'grid', gap: 6 }}>
              {history.map((h) => {
                ...
                return (
                  <div key={h.instrument_index_id} style={{
                    display: 'flex', alignItems: 'center', gap: 8,
                    padding: '8px 10px', borderRadius: 8,
                    border: `1px solid ${active ? ACTIVE_BORDER : THEME_BORDER}`,
                    background: active ? ACTIVE_BG : '#fff',
                  }}>
                    <Gauge size={12} color={active ? ACTIVE_ICON : THEME_MUTED} />
                    ...
                    {active
                      ? <span style={{
                          ...
                          color: ACTIVE_TEXT, background: '#fff', border: `1px solid ${ACTIVE_BORDER}`,
                        }}>
                          <CheckCircle2 size={10} /> active
                        </span>
                      : <button
                          type="button" onClick={() => onActivate(h.instrument_index_id)}
                          style={{
                    ...
                    <button
                      type="button" onClick={() => onDelete(h.instrument_index_id)}
                      title="Delete"
                      style={{
                        display: 'flex', alignItems: 'center',
