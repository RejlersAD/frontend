 ...
 *
 * Lightweight, RBAC-aware project organiser for the Non-TEFF Metadata
 * Generator. Mirrors the visual language of NonTeffMetadataPage (emerald /
 * teal) but stays self-contained — no business-logic dependency on the
 * extraction page.
 *
...
const PROJECT_PAGE_CFG = {
  api: {
    ...
    detail: (id) => `/non-teff/projects/${id}/`,
    items:  (id) => `/non-teff/projects/${id}/items/`,
  },
  routes: {
    metadataPage: '/engineering/digitization/non-teff-metadata',
  },
  statuses: [
    { value: 'active',    label: 'Active',    color: '#059669', bg: 'rgba(5,150,105,0.10)' },
...
const NonTeffProjectsPage = () => {
  ...
  const handleOpen = (p) => {
    // Persist the active project so the extraction page can pick it up.
    try {
      localStorage.setItem('nonTeffActiveProject', JSON.stringify({
        project_id: p.project_id, name: p.name, plant: p.plant,
      }));
    } catch (_) { /* ignore */ }
    navigate(PROJECT_PAGE_CFG.routes.metadataPage);
  };
    ...
    <div style={{ minHeight: '100vh', background: T.pageBg, padding: '24px 32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <button
            onClick={() => navigate(PROJECT_PAGE_CFG.routes.metadataPage)}
            style={{
          ...
          <div>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 13, color: T.muted }}>
              Organise extractions by engineering project. Role-based — you see what you create; admins see all.
            </p>
          </div>
        </div>
        <button
...
const ProjectCard = ({ project, theme, onOpen, onEdit, onDelete }) => {
  ...
  return (
    ...
    onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(5,150,105,0.12)'; }}
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        ...
        <div style={{ fontSize: 11, color: theme.muted, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {project.job_count + project.batch_count} extraction{(project.job_count + project.batch_count) === 1 ? '' : 's'}
        </div>
...
const EmptyState = ({ onCreate, theme }) => (
  <div style={{
    ...
    <p style={{ margin: '6px 0 18px', fontSize: 13, color: theme.muted }}>
      Create a project to group related extractions (single-file jobs and bulk Master Index batches).
    </p>
...
const ProjectFormModal = ({ initial, onClose, onSubmit, busy, theme }) => {
  ...
  // When editing, auto-open advanced if any advanced field is non-empty.
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: '2px 0 0', fontSize: 12, color: theme.muted }}>
              {initial ? 'Update the project details.' : 'Group your Non-TEFF extractions under one project.'}
            </p>
          </div>
          ...
          <button
            ...
            style={{
              background: canSubmit
                ? `linear-gradient(135deg, ${theme.accent}, #0891b2)`
                : '#cbd5e1',
              color: '#fff', border: 'none',
              padding: '9px 20px', borderRadius: 8, fontSize: 13, fontWeight: 700,
              cursor: canSubmit ? 'pointer' : 'not-allowed',
              display: 'inline-flex', alignItems: 'center', gap: 6,
              boxShadow: canSubmit ? '0 4px 14px rgba(5,150,105,0.30)' : 'none',
            }}
          >
...
export default NonTeffProjectsPage;
